...
fn test_from_gregorian() {
    ...
    assert_eq!(utc_epoch.into_time_scale(TimeScale::TAI), tai_epoch);
    assert_eq!(utc_epoch, tai_epoch.into_time_scale(TimeScale::UTC));
...
fn unix() {
    ...
    assert_eq!(
        format!("{}", unix_epoch.into_time_scale(TimeScale::UTC)),
        "1970-01-01T00:00:00 UTC"
    );
    assert_eq!(
        format!("{:x}", unix_epoch.into_time_scale(TimeScale::TAI)),
...
fn test_add_durations_over_leap_seconds() {
    ...
    let pre_ls_tai = pre_ls_utc.into_time_scale(TimeScale::TAI);
...
fn test_weekday() {
    ...
    let permutate_time_scale = |e: Epoch, expect: Weekday| {
        for new_time_scale in [
            ...
            let e_ts = e.into_time_scale(new_time_scale);
...
fn test_get_time() {
    ...
    let other = other_utc.into_time_scale(TimeScale::TDB);
...
fn test_time_of_week() {
    ...
    // TAI<=>UTC
    let epoch_utc = epoch.into_time_scale(TimeScale::UTC);
    ...
    let epoch_qzsst = epoch.into_time_scale(TimeScale::QZSST);
...
fn test_day_of_year() {
    ...
    let recip_func = |utc_epoch: Epoch| {
        ...
        for ts in &[
            ...
            let epoch = utc_epoch.into_time_scale(*ts);
...
fn test_to_tai_time_scale() {
    ...
    assert_eq!(j1900_ref, j1900_ref.into_time_scale(TimeScale::TAI));
    let j2000_ref = J2000_REF_EPOCH;
    assert_eq!(j2000_ref, j2000_ref.into_time_scale(TimeScale::TAI));
...
fn test_to_utc_time_scale() {
    ...
    let gpst_utc_ref = gpst_tai_ref.into_time_scale(TimeScale::UTC);
    ...
    let bdt_utc_ref = bdt_tai_ref.into_time_scale(TimeScale::UTC);
