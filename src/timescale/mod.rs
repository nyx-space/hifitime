...
mod bolero_harnesses {
    ...
    fn bolero_test_194_default() {
        bolero::check!()
            .with_type()
            .cloned()
            .for_each(|()| Some(TimeScale::default()));
    }

    #[test]
    #[cfg_attr(kani, kani::proof)]
    fn bolero_test_195_formatted_len() {
        bolero::check!()
            .with_type()
            .cloned()
            .for_each(|callee: TimeScale| Some(callee.formatted_len()));
    }

    #[test]
    #[cfg_attr(kani, kani::proof)]
    fn bolero_test_196_is_gnss() {
        bolero::check!()
            .with_type()
            .cloned()
            .for_each(|callee: TimeScale| Some(callee.is_gnss()));
    }

    #[test]
    #[cfg_attr(kani, kani::proof)]
    fn bolero_test_197_reference_epoch() {
        bolero::check!()
            .with_type()
            .cloned()
            .for_each(|callee: TimeScale| Some(callee.reference_epoch()));
    }

    #[test]
    #[cfg_attr(kani, kani::proof)]
    fn bolero_test_198_prime_epoch_offset() {
        bolero::check!()
            .with_type()
            .cloned()
            .for_each(|callee: TimeScale| Some(callee.prime_epoch_offset()));
    }

    #[test]
    #[cfg_attr(kani, kani::proof)]
    fn bolero_test_199_gregorian_epoch_offset() {
        bolero::check!()
            .with_type()
            .cloned()
            .for_each(|callee: TimeScale| Some(callee.gregorian_epoch_offset()));
    }

    #[test]
    #[cfg_attr(kani, kani::proof)]
    fn bolero_test_200_uses_leap_seconds() {
        bolero::check!()
            .with_type()
            .cloned()
            .for_each(|callee: TimeScale| Some(callee.uses_leap_seconds()));
    }

    #[test]
    #[cfg_attr(kani, kani::proof)]
    fn bolero_test_201_from() {
        bolero::check!()
            .with_type()
            .cloned()
            .for_each(|val: u8| Some(TimeScale::from(val)));
    }
}
