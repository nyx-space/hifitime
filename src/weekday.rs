...
mod bolero_harnesses {
    ...
    fn bolero_test_217_default() {
        bolero::check!()
            .with_type()
            .cloned()
            .for_each(|()| Some(Weekday::default()));
    }

    #[test]
    #[cfg_attr(kani, kani::proof)]
    fn bolero_test_218_to_c89_weekday() {
        bolero::check!()
            .with_type()
            .cloned()
            .for_each(|callee: Weekday| Some(callee.to_c89_weekday()));
    }

    #[test]
    #[cfg_attr(kani, kani::proof)]
    fn bolero_test_219_from() {
        bolero::check!()
            .with_type()
            .cloned()
            .for_each(|u: u8| Some(Weekday::from(u)));
    }

    #[test]
    #[cfg_attr(kani, kani::proof)]
    fn bolero_test_220_from() {
        bolero::check!()
            .with_type()
            .cloned()
            .for_each(|i: i8| Some(Weekday::from(i)));
    }

    #[test]
    #[cfg_attr(kani, kani::proof)]
    fn bolero_test_221_add() {
        bolero::check!()
            .with_type()
            .cloned()
            .for_each(|(callee, rhs): (Weekday, Weekday)| Some(callee.add(rhs)));
    }

    #[test]
    #[cfg_attr(kani, kani::proof)]
    fn bolero_test_222_sub() {
        bolero::check!()
            .with_type()
            .cloned()
            .for_each(|(callee, rhs): (Weekday, Weekday)| Some(callee.sub(rhs)));
    }

    #[test]
    #[cfg_attr(kani, kani::proof)]
    fn bolero_test_223_add() {
        bolero::check!()
            .with_type()
            .cloned()
            .for_each(|(callee, rhs): (Weekday, u8)| Some(callee.add(rhs)));
    }

    #[test]
    #[cfg_attr(kani, kani::proof)]
    fn bolero_test_224_sub() {
        bolero::check!()
            .with_type()
            .cloned()
            .for_each(|(callee, rhs): (Weekday, u8)| Some(callee.sub(rhs)));
    }

    #[test]
    #[cfg_attr(kani, kani::proof)]
    fn bolero_test_225_add_assign() {
        bolero::check!()
            .with_type()
            .cloned()
            .for_each(|(mut callee, rhs): (Weekday, u8)| Some(callee.add_assign(rhs)));
    }

    #[test]
    #[cfg_attr(kani, kani::proof)]
    fn bolero_test_226_sub_assign() {
        bolero::check!()
            .with_type()
            .cloned()
            .for_each(|(mut callee, rhs): (Weekday, u8)| Some(callee.sub_assign(rhs)));
    }
}
