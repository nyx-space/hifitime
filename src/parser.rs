...
mod bolero_harnesses {
    ...
    fn bolero_test_14_default() {
        bolero::check!()
            .with_type()
            .cloned()
            .for_each(|()| Some(Token::default()));
    }

    #[test]
    #[cfg_attr(kani, kani::proof)]
    fn bolero_test_15_value_ok() {
        bolero::check!()
            .with_type()
            .cloned()
            .for_each(|(callee, val): (Token, i32)| Some(callee.value_ok(val)));
    }

    #[test]
    #[cfg_attr(kani, kani::proof)]
    fn bolero_test_16_gregorian_position() {
        bolero::check!()
            .with_type()
            .cloned()
            .for_each(|callee: Token| Some(callee.gregorian_position()));
    }

    #[test]
    #[cfg_attr(kani, kani::proof)]
    fn bolero_test_17_advance_with() {
        bolero::check!().with_type().cloned().for_each(
            |(mut callee, ending_char): (Token, char)| Some(callee.advance_with(ending_char)),
        );
    }

    #[test]
    #[cfg_attr(kani, kani::proof)]
    fn bolero_test_18_is_numeric() {
        bolero::check!()
            .with_type()
            .cloned()
            .for_each(|callee: Token| Some(callee.is_numeric()));
    }
}
