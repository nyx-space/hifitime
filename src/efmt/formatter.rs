...
mod bolero_harnesses {
    ...
    #[test]
    #[cfg_attr(kani, kani::proof)]
    fn bolero_test_10_sep_char_is() {
        bolero::check!()
            .with_type()
            .cloned()
            .for_each(|(callee, c_in): (Item, char)| Some(callee.sep_char_is(c_in)));
    }

    #[test]
    #[cfg_attr(kani, kani::proof)]
    fn bolero_test_11_sep_char_is_not() {
        bolero::check!()
            .with_type()
            .cloned()
            .for_each(|(callee, c_in): (Item, char)| Some(callee.sep_char_is_not(c_in)));
    }

    #[test]
    #[cfg_attr(kani, kani::proof)]
    fn bolero_test_12_second_sep_char_is() {
        bolero::check!()
            .with_type()
            .cloned()
            .for_each(|(callee, c_in): (Item, char)| Some(callee.second_sep_char_is(c_in)));
    }

    #[test]
    #[cfg_attr(kani, kani::proof)]
    fn bolero_test_13_second_sep_char_is_not() {
        bolero::check!()
            .with_type()
            .cloned()
            .for_each(|(callee, c_in): (Item, char)| Some(callee.second_sep_char_is_not(c_in)));
    }
}
