...
mod bolero_harnesses {
    ...
    #[test]
    #[cfg_attr(kani, kani::proof)]
    fn bolero_test_213_next() {
        bolero::check!()
            .with_type()
            .cloned()
            .for_each(|mut callee: TimeSeries| Some(callee.next()));
    }

    #[test]
    #[cfg_attr(kani, kani::proof)]
    fn bolero_test_214_size_hint() {
        bolero::check!()
            .with_type()
            .cloned()
            .for_each(|callee: TimeSeries| Some(callee.size_hint()));
    }

    #[test]
    #[cfg_attr(kani, kani::proof)]
    fn bolero_test_215_next_back() {
        bolero::check!()
            .with_type()
            .cloned()
            .for_each(|mut callee: TimeSeries| Some(callee.next_back()));
    }

    #[test]
    #[cfg_attr(kani, kani::proof)]
    fn bolero_test_216_len() {
        bolero::check!()
            .with_type()
            .cloned()
            .for_each(|callee: TimeSeries| Some(callee.len()));
    }
}
