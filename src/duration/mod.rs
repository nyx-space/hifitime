...
mod bolero_harnesses {
    ...
    fn bolero_test_159_eq() {
        bolero::check!()
            .with_type()
            .cloned()
            .for_each(|(callee, other): (Duration, Duration)| Some(callee.eq(&other)));
    }

    /*
    #[test]
    #[cfg_attr(kani, kani::proof)]
    fn bolero_test_160_hash() {
        bolero::check!().with_type().cloned().for_each(|(callee, hasher): (Duration , H/#0)| Some(callee.hash(&hasher)));
    }
    ...
    */
    #[test]
    #[cfg_attr(kani, kani::proof)]
    fn bolero_test_161_default() {
        bolero::check!()
            .with_type()
            .cloned()
            .for_each(|()| Some(Duration::default()));
    }

    #[test]
    #[cfg_attr(kani, kani::proof)]
    fn bolero_test_162_from_parts() {
        bolero::check!()
            .with_type()
            .cloned()
    ...
    #[test]
    #[cfg_attr(kani, kani::proof)]
    fn bolero_test_163_from_total_nanoseconds() {
        bolero::check!()
            .with_type()
            .cloned()
    ...
    #[test]
    #[cfg_attr(kani, kani::proof)]
    fn bolero_test_164_from_truncated_nanoseconds() {
        bolero::check!()
            .with_type()
            .cloned()
    ...
    #[test]
    #[cfg_attr(kani, kani::proof)]
    fn bolero_test_167_from_seconds() {
        bolero::check!()
            .with_type()
            .cloned()
    ...
    #[test]
    #[cfg_attr(kani, kani::proof)]
    fn bolero_test_168_from_milliseconds() {
        bolero::check!()
            .with_type()
            .cloned()
    ...
    #[test]
    #[cfg_attr(kani, kani::proof)]
    fn bolero_test_169_from_microseconds() {
        bolero::check!()
            .with_type()
            .cloned()
    ...
    #[test]
    #[cfg_attr(kani, kani::proof)]
    fn bolero_test_170_from_nanoseconds() {
        bolero::check!()
            .with_type()
            .cloned()
    ...
    #[test]
    #[cfg_attr(kani, kani::proof)]
    fn bolero_test_172_compose_f64() {
        bolero::check!().with_type().cloned().for_each(
            |(sign, days, hours, minutes, seconds, milliseconds, microseconds, nanoseconds): (
                i8,
                f64,
    ...
    #[test]
    #[cfg_attr(kani, kani::proof)]
    fn bolero_test_173_from_tz_offset() {
        bolero::check!()
            .with_type()
            .cloned()
    ...
    #[test]
    #[cfg_attr(kani, kani::proof)]
    fn bolero_test_174_normalize() {
        bolero::check!()
            .with_type()
            .cloned()
            .for_each(|mut callee: Duration| Some(callee.normalize()));
    }

    #[test]
    #[cfg_attr(kani, kani::proof)]
    fn bolero_test_175_to_parts() {
        bolero::check!()
            .with_type()
            .cloned()
            .for_each(|callee: Duration| Some(callee.to_parts()));
    }

    #[test]
    #[cfg_attr(kani, kani::proof)]
    fn bolero_test_176_total_nanoseconds() {
        bolero::check!()
            .with_type()
            .cloned()
            .for_each(|callee: Duration| Some(callee.total_nanoseconds()));
    }

    #[test]
    #[cfg_attr(kani, kani::proof)]
    fn bolero_test_177_try_truncated_nanoseconds() {
        bolero::check!()
            .with_type()
            .cloned()
            .for_each(|callee: Duration| Some(callee.try_truncated_nanoseconds()));
    }

    #[test]
    #[cfg_attr(kani, kani::proof)]
    fn bolero_test_178_truncated_nanoseconds() {
        bolero::check!()
            .with_type()
            .cloned()
            .for_each(|callee: Duration| Some(callee.truncated_nanoseconds()));
    }

    #[test]
    #[cfg_attr(kani, kani::proof)]
    fn bolero_test_179_to_seconds() {
        bolero::check!()
            .with_type()
            .cloned()
            .for_each(|callee: Duration| Some(callee.to_seconds()));
    }

    #[test]
    #[cfg_attr(kani, kani::proof)]
    fn bolero_test_180_to_unit() {
        bolero::check!()
            .with_type()
            .cloned()
            .for_each(|(callee, unit): (Duration, Unit)| Some(callee.to_unit(unit)));
    }

    #[test]
    #[cfg_attr(kani, kani::proof)]
    fn bolero_test_181_abs() {
        bolero::check!()
            .with_type()
            .cloned()
            .for_each(|callee: Duration| Some(callee.abs()));
    }

    #[test]
    #[cfg_attr(kani, kani::proof)]
    fn bolero_test_182_signum() {
        bolero::check!()
            .with_type()
            .cloned()
            .for_each(|callee: Duration| Some(callee.signum()));
    }

    #[test]
    #[cfg_attr(kani, kani::proof)]
    fn bolero_test_183_decompose() {
        bolero::check!()
            .with_type()
            .cloned()
            .for_each(|callee: Duration| Some(callee.decompose()));
    }

    #[test]
    #[cfg_attr(kani, kani::proof)]
    fn bolero_test_184_subdivision() {
        bolero::check!()
            .with_type()
            .cloned()
            .for_each(|(callee, unit): (Duration, Unit)| Some(callee.subdivision(unit)));
    }

    #[test]
    #[cfg_attr(kani, kani::proof)]
    fn bolero_test_185_floor() {
        bolero::check!()
            .with_type()
            .cloned()
            .for_each(|(callee, duration): (Duration, Duration)| Some(callee.floor(duration)));
    }

    #[test]
    #[cfg_attr(kani, kani::proof)]
    fn bolero_test_186_ceil() {
        bolero::check!()
            .with_type()
            .cloned()
            .for_each(|(callee, duration): (Duration, Duration)| Some(callee.ceil(duration)));
    }

    #[test]
    #[cfg_attr(kani, kani::proof)]
    fn bolero_test_187_round() {
        bolero::check!()
            .with_type()
            .cloned()
            .for_each(|(callee, duration): (Duration, Duration)| Some(callee.round(duration)));
    }

    #[test]
    #[cfg_attr(kani, kani::proof)]
    fn bolero_test_188_approx() {
        bolero::check!()
            .with_type()
            .cloned()
            .for_each(|callee: Duration| Some(callee.approx()));
    }

    #[test]
    #[cfg_attr(kani, kani::proof)]
    fn bolero_test_189_min() {
        bolero::check!()
            .with_type()
            .cloned()
            .for_each(|(callee, other): (Duration, Duration)| Some(callee.min(other)));
    }

    #[test]
    #[cfg_attr(kani, kani::proof)]
    fn bolero_test_190_max() {
        bolero::check!()
            .with_type()
            .cloned()
            .for_each(|(callee, other): (Duration, Duration)| Some(callee.max(other)));
    }

    #[test]
    #[cfg_attr(kani, kani::proof)]
    fn bolero_test_191_is_negative() {
        bolero::check!()
            .with_type()
            .cloned()
            .for_each(|callee: Duration| Some(callee.is_negative()));
    }

    #[test]
    #[cfg_attr(kani, kani::proof)]
    fn bolero_test_192_eq() {
        bolero::check!()
            .with_type()
            .cloned()
            .for_each(|(callee, unit): (Duration, Unit)| Some(callee.eq(&unit)));
    }

    #[test]
    #[cfg_attr(kani, kani::proof)]
    fn bolero_test_193_partial_cmp() {
        bolero::check!()
            .with_type()
            .cloned()
            .for_each(|(callee, unit): (Duration, Unit)| Some(callee.partial_cmp(&unit)));
    }
}
