...
mod bolero_harnesses {
    ...
    fn bolero_test_202_default() {
        bolero::check!()
            .with_type()
            .cloned()
            .for_each(|()| Some(Unit::default()));
    }

    #[test]
    #[cfg_attr(kani, kani::proof)]
    fn bolero_test_203_default() {
        bolero::check!()
            .with_type()
            .cloned()
            .for_each(|()| Some(Freq::default()));
    }

    #[test]
    #[cfg_attr(kani, kani::proof)]
    fn bolero_test_204_add() {
        bolero::check!()
            .with_type()
            .cloned()
            .for_each(|(callee, rhs): (Unit, Unit)| Some(callee.add(rhs)));
    }

    #[test]
    #[cfg_attr(kani, kani::proof)]
    fn bolero_test_205_sub() {
        bolero::check!()
            .with_type()
            .cloned()
            .for_each(|(callee, rhs): (Unit, Unit)| Some(callee.sub(rhs)));
    }

    #[test]
    #[cfg_attr(kani, kani::proof)]
    fn bolero_test_206_in_seconds() {
        bolero::check!()
            .with_type()
            .cloned()
            .for_each(|callee: Unit| Some(callee.in_seconds()));
    }

    #[test]
    #[cfg_attr(kani, kani::proof)]
    fn bolero_test_207_from_seconds() {
        bolero::check!()
            .with_type()
            .cloned()
            .for_each(|callee: Unit| Some(callee.from_seconds()));
    }

    #[test]
    #[cfg_attr(kani, kani::proof)]
    fn bolero_test_208_from() {
        bolero::check!()
            .with_type()
            .cloned()
            .for_each(|val: u8| Some(Unit::from(val)));
    }

    #[test]
    #[cfg_attr(kani, kani::proof)]
    fn bolero_test_209_mul() {
        bolero::check!()
            .with_type()
            .cloned()
            .for_each(|(callee, q): (Unit, i64)| Some(callee.mul(q)));
    }

    #[test]
    #[cfg_attr(kani, kani::proof)]
    fn bolero_test_210_mul() {
        bolero::check!()
            .with_type()
            .cloned()
            .for_each(|(callee, q): (Unit, f64)| Some(callee.mul(q)));
    }
}
